...
import org.openhab.binding.alarmclock.internal.system.CompactTime;
import org.openhab.binding.alarmclock.internal.system.DayOfWeek;
...
public abstract class AbstractClockHandler extends BaseThingHandler {
    ...
    protected Logger logger = LoggerFactory.getLogger(AbstractClockHandler.class);
    ...
    protected void initEventHandlers() {
        ...
        clockManager.once(Event.MINUTE_TICK, (previous, current) -> {
            ...
            if (onTime.isLessThan(offTime)) {
                switchStatus((onTime.isLessThan(current) && (current.isLessThanOrEqual(offTime))) ? OnOffType.ON
                        : OnOffType.OFF);
            } else {
                switchStatus((offTime.isLessThan(current) && (current.isLessThanOrEqual(onTime))) ? OnOffType.OFF
                        : OnOffType.ON);
            }
        }, this);
        ...
        clockManager.on(Event.MINUTE_TICK, (previous, current) -> {
            if (isEnabled() && isDayEnabled() && isValid()) {
